...
{
	private final JSONFlightServiceAsync jsonFlightService = GWT.create( JSONFlightService.class );

	public void onModuleLoad()
	{
		final double start = Duration.currentTimeMillis();
		jsonFlightService.getFlighDelayData( new AsyncCallback<ArrayList<FlightDelayData>>()
		{
			public void onSuccess( ArrayList<FlightDelayData> data )
			{
				double end = Duration.currentTimeMillis();
				GWT.log( "Done RPC in " + ( end - start ) / 1000 + " Seconds" );
		...
		jsonFlightService.getMetricData( new AsyncCallback<ArrayList<MetricData>>()
		{
			public void onSuccess( ArrayList<MetricData> data )
			{
				double end = Duration.currentTimeMillis();
				GWT.log( "Done RPC in " + ( end - start ) / 1000 + " Seconds" );
				Crossfilter<MetricData> metrics = initMetricCrossFilter( data );
			}
	...
	protected void initWidgets( ArrayList<Dimension<FlightDelayData, ? extends Object>> flightDimensions )
	{
	}

	private Crossfilter<MetricData> initMetricCrossFilter( ArrayList<MetricData> data )
	{
		final Crossfilter<MetricData> metrics = Crossfilter.create( data );
		return metrics;
	}

	private Crossfilter<FlightDelayData> initCrossFilter( ArrayList<FlightDelayData> data )
	{
		...
		GWT.log( "Done Crossfilter in " + ( endCrossfilter - startCrossfilter ) / 1000 + " Seconds" );
		...
		flightDelayData.setDate( new Date( data.get( 0 ).getDate().getTime() + 36000 ) );
		...
		GWT.log( "Done AddCrossfilter in " + ( endAddCrossfilter - startAddCrossfilter ) / 1000 + " Seconds" );
		GWT.log( "Crossfilter Size " + flights.size() );
		return flights;
	}

	protected ArrayList<Dimension<FlightDelayData, ? extends Object>> initDimensions( Crossfilter<FlightDelayData> flights )
	{
		final double startAddDimensions = Duration.currentTimeMillis();

		final Dimension<FlightDelayData, Date> date = flights.dimension( new DateReducer<FlightDelayData>()
		{
		...
		final Dimension<FlightDelayData, Double> hour = flights.dimension( new DoubleReducer<FlightDelayData>()
		{
			public double getValue( FlightDelayData object )
			{
				return object.getDate().getHours() + object.getDate().getMinutes() / 60;
			}
		} );
		Dimension<FlightDelayData, Integer> delay = flights.dimension( new IntReducer<FlightDelayData>()
		{
		...
		Dimension<FlightDelayData, Integer> distance = flights.dimension( new IntReducer<FlightDelayData>()
		{
			public int getValue( FlightDelayData object )
			{
		...
		final Dimension<FlightDelayData, String> origin = flights.dimension( new StringReducer<FlightDelayData>()
		{
		...
		final Dimension<FlightDelayData, String> destination = flights.dimension( new StringReducer<FlightDelayData>()
		{
			public String getValue( FlightDelayData object )
			{
		...
		GWT.log( "Done AddDimensions in " + ( endAddDimensions - startAddDimensions ) / 1000 + " Seconds" );
		...
		Group<FlightDelayData, Date> dates = date.group( CrossfilterUtil.createGroupingDays() );
		Group<FlightDelayData, Double> hours = hour.group( CrossfilterUtil.createGroupingToFloor() );
		Group<FlightDelayData, Integer> delays = delay.group( CrossfilterUtil.createGroupingToFloor( 10 ) );
		Group<FlightDelayData, Integer> distances = distance.group( CrossfilterUtil.createGroupingToFloor( 50 ) );

		final double endAddGroups = Duration.currentTimeMillis();
		GWT.log( "Done AddGroups in " + ( startAddGroups - endAddGroups ) / 1000 + " Seconds" );

		ValueListBox<FlightDelayData> originwidget = new ValueListBox<FlightDelayData>( new Renderer<FlightDelayData>()
		{
			...
			@Override
			public void render( FlightDelayData object, Appendable appendable ) throws IOException
			{
